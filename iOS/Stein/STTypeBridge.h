...
///Look up the Human Readable type for a specified ObjC type.
ST_EXTERN NSString *STTypeBridgeGetHumanReadableTypeForObjCType(NSString *type);
...
/// \param  scope   The scope to look up class names under.
///
/// \result An Objective-C type string.
///
///Human-readable types are currently only used in method definitions.
ST_EXTERN NSString *STTypeBridgeGetObjCTypeForHumanReadableType(NSString *type, STScope *scope);
