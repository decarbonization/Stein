...
{
	return [NSString stringWithFormat:@"<%@:%p { %d, %d }>", [self className], self, mRange.location, mRange.length];
}

- (NSString *)prettyDescription
{
	return [NSString stringWithFormat:@"range %d %d", mRange.location, mRange.length];
}
...
static BOOL STRangeCanWrapValueWithSignature(const STPrimitiveValueWrapperDescriptor *descriptor, const char *objcType)
{
	if(sizeof(NSRange) == sizeof(CFRange))
		return CStringHasPrefix(objcType, "{_NSRange=") || CStringHasPrefix(objcType, "{CFRange=");
	   
	   return CStringHasPrefix(objcType, "{_NSRange=");
}
...
{
	return CStringHasPrefix(objcType, "{CGPoint=");
}
...
{
	return CStringHasPrefix(objcType, "{CGSize=");
}
...
{
	return [[STSize alloc] initWithSize:*(CGSize *)data];
}
...
{
	return CStringHasPrefix(objcType, "{CGRect=");
}
