...
- (id)applyWithArguments:(STList *)arguments inScope:(STScope *)superscope
{
	...
	for (id name in mPrototype)
	{
		if(index >= countOfArguments)
			[scope setValue:STNull forConstantNamed:[name string]];
		else
			[scope setValue:[arguments objectAtIndex:index] forConstantNamed:[name string]];
		index++;
	}
...
- (BOOL)isEqual:(id)object
{
	...
	{
		return ([self.prototype isEqual:[object prototype]] && 
				[self.implementation isEqual:[object implementation]] && 
				[self.closureSignature isEqual:[object closureSignature]]);
	}
