...
- (id)initWithValue:(void *)value ofType:(const char *)objcType
{
	...
	if((self = [super init]))
	{
		...
		mValue = malloc(mSizeOfValue);
		mObjcType = malloc(strlen(objcType) + 1);
...
{
    NSUInteger sizeOfBuffer = 0;
    NSGetSizeAndAlignment(objcType, &sizeOfBuffer, NULL);
	NSAssert((sizeOfBuffer == mSizeOfValue), 
			 @"Buffer given to generic struct descriptor is %zd bytes, but the generic struct descriptor's buffer is %zd bytes.", sizeOfBuffer, mSizeOfValue);
...
///Search a C string for a matching closing character, ignoring any nested character-pairs.
/// \param	openChar	The opening character that describes the beginning of a nested character-pair.
/// \param	closeChar	The closing character that describes the end of nested character-pairs,
///                     as well as the initial search started when invoking this function.
/// \param	start		The index to start the search at.
/// \param	string		The string to search.
///
/// \result	The location of the final closing character, or -1 if it could not be found.
static int FindClosingCharacter(unichar openChar, unichar closeChar, int start, const char *string)
{
			...
			if(indexOfEqualsCharacter != -1)
			{
				...
				else if((typeCharacter == '[') || (typeCharacter == '('))
				{
					...
					[NSException raise:NSInternalInconsistencyException 
								format:@"Unexpected union or C array encountered while caching a C struct's memory layout. Unions and C arrays are not supported by Stein."];
				}
...
ffi_type *STTypeBridgeConvertObjCTypeToFFIType(const char *objcType)
{
	...
	switch (type[0])
	{
		...
		default:
			[NSException raise:NSInternalInconsistencyException 
						format:@"Type unknown to Stein encountered."];
	}
...
NSString *STTypeBridgeGetHumanReadableTypeForObjCType(NSString *type)
{
    unichar firstCharacterOfType = [type characterAtIndex:0];
	if(firstCharacterOfType == kObjectiveCTypeModifierConst || firstCharacterOfType == kObjectiveCTypeModifierIn ||
	   firstCharacterOfType == kObjectiveCTypeModifierInout || firstCharacterOfType == kObjectiveCTypeModifierOut ||
	   firstCharacterOfType == kObjectiveCTypeModifierBycopy || firstCharacterOfType == kObjectiveCTypeModifierByref ||
	   firstCharacterOfType == kObjectiveCTypeModifierOneway)
	{
		return STTypeBridgeGetHumanReadableTypeForObjCType([type substringFromIndex:1]);
	}
    
    if([type hasPrefix:@"^"])
        return [STTypeBridgeGetHumanReadableTypeForObjCType([type substringFromIndex:1]) stringByAppendingString:@"*"];
...
NSString *STTypeBridgeGetObjCTypeForHumanReadableType(NSString *type, STScope *scope)
{
	if([type hasSuffix:@"*"] || [type hasSuffix:@"[]"])
		return [@"^" stringByAppendingString:STTypeBridgeGetObjCTypeForHumanReadableType([type substringToIndex:[type length] - 1], scope)];
	...
	if([type isEqualToString:@"char"] || [type isEqualToString:@"BOOL"])
		...
		return @"#";
	else if([type isEqualToString:@"id"] || (NSClassFromString(type) != nil) || class_isMetaClass([scope valueForVariableNamed:type searchParentScopes:YES]))
		return @"@";
