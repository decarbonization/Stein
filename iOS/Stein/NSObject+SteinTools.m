...
+ (void)load
{
    [self alias:@"+alias:as:" as:@"+aliasas"];
    [self alias:@"+implements:" as:@"+implements"];
}

#pragma mark - Class Tools

+ (void)implements:(NSArray *)protocols
{
    for (id name in protocols)
    {
        Protocol *protocol = objc_getProtocol([[name string] UTF8String]);
        if(protocol)
            class_addProtocol(self, protocol);
        else
            STRaiseIssue(nil, @"Unknown protocol named %@", [name string]);
    }
}

+ (void)alias:(NSString *)originalName as:(NSString *)newName
{
    SEL originalSelector = NSSelectorFromString([originalName substringFromIndex:1]);
    SEL newSelector = NSSelectorFromString([newName substringFromIndex:1]);
    
    if([originalName hasPrefix:@"+"] && [newName hasPrefix:@"+"])
    {
        Method method = class_getClassMethod(self, originalSelector);
        class_addMethod(object_getClass(self), newSelector, method_getImplementation(method), method_getTypeEncoding(method));
    }
    else if([originalName hasPrefix:@"-"] && [newName hasPrefix:@"-"])
    {
        Method method = class_getInstanceMethod(self, originalSelector);
        class_addMethod(self, newSelector, method_getImplementation(method), method_getTypeEncoding(method));
    }
    else
    {
        STRaiseIssue(nil, @"alias given malformed names %@ and %@", originalName, newName);
    }
}
...
- (NSString *)prettyPrint
{
    ...
    [[PRTConsoleViewController sharedConsoleViewController] writeLine:prettyDescription];
...
- (NSString *)print
{
	...
	[[PRTConsoleViewController sharedConsoleViewController] writeLine:description];
...
- (id)map:(id <STFunction>)function
{
	...
	for (NSUInteger index = 0, length = [self length]; index < length; index++)
	{
		@try
		{
			...
			{
				[string appendFormat:@"%hhd", [result charValue]];
			}
			else
			{
...
- (id)filter:(id <STFunction>)function
{
	...
	for (NSUInteger index = 0, length = [self length]; index < length; index++)
	{
		@try
		{
			...
			{
				[string appendFormat:@"%hhd", [character charValue]];
			}
		}
		@catch (STBreakException *e)
		{
...
- (NSArray *)where:(NSArray *)booleans
{
	...
	NSAssert(([self count] <= [booleans count]), 
			 @"Wrong number of values given to where, expected at least #d got #d", [self count], [booleans count]);
...
- (NSString *)prettyDescription
{
	...
	[self enumerateIndexesUsingBlock:^(NSUInteger index, BOOL *stop) {
		[description appendFormat:@" %d", index];
	}];
