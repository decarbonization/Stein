...
///Returns the current scope being used in the current thread.
///
///This function will only return a meaningful value for as long as
///there is an object being messaged.
ST_EXTERN STScope *STObjectBridgeGetCurrentScope();
...
///
///The object bridge messaging mechanism adds an additional route to find a handler for
///selectors the `target` object does not recognize:
///
///-	First, it will attempt to use the forwarding mechanism defined by STMethodMissing.
...
ST_EXTERN void STExtendClass(Class classToExtend, STList *expressions, STScope *scope);

#pragma mark -

///Returns a BOOL indicating whether or not a class was defined in Stein.
ST_EXTERN BOOL STClassIsDefinedInStein(Class class);
